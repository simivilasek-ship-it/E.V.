...
class TestCommandExecutor:
    ...
    def test_init(self, command_executor, mock_config):
        ...
        assert isinstance(command_executor.is_windows, bool)
        assert isinstance(command_executor.is_linux, bool)
    ...
    def test_open_app_chrome(self, mock_popen, command_executor):
        ...
        with patch.object(command_executor, "_find_app", return_value="chrome"):
            result = command_executor._cmd_open_app(app="chrome")
            assert result == "ok"
            mock_popen.assert_called_once()
    
    @patch('commands.subprocess.Popen')
    def test_open_app_not_found(self, mock_popen, command_executor):
        """Test opening non-existent app"""
        with patch.object(command_executor, "_find_app", return_value=None):
            result = command_executor._cmd_open_app(app="nonexistent")
            assert "nenalezena" in result
            mock_popen.assert_not_called()
    
    @patch('commands.pyautogui.screenshot')
    def test_screenshot(self, mock_screenshot, command_executor):
        """Test screenshot command"""
        with patch("commands.HAS_PYAUTOGUI", True):
    ...
    def test_cmd_calculate_simple(self, command_executor):
        ...
        assert command_executor._cmd_calculate(expr="2+2") == "4"
        assert command_executor._cmd_calculate(expr="10-5") == "5"
        assert command_executor._cmd_calculate(expr="3*4") == "12"
    
    def test_cmd_calculate_invalid(self, command_executor):
        """Test calculate with invalid expression"""
        result = command_executor._cmd_calculate(expr="invalid;;;___")
    ...
    @patch('commands.pyautogui.press')
    ...
    @patch('commands.pyautogui.typewrite')
...
class TestCommandIntegration:
    ...
    @patch('commands.psutil.cpu_percent')
    @patch('commands.psutil.virtual_memory')
