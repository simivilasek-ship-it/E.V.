...
FRAMES = [
    ("E.V. v5.4 — Copilot · Agent · PC", [
        "You:  přehled o PC",
        "E.V.: 🖥️ CPU 12% | RAM 32% | Cursor aktivní",
    ]),
    ("", [
        "You:  co mám na obrazovce?",
        "E.V.: V popředí Cursor — Firefox otevřený",
    ]),
    ("", [
        "You:  jaké je počasí v Praze?",
        "E.V.: ⛅ Praha: Polojasno, 21°C",
    ]),
    ("", [
        "You:  najdi async knihovny a ulož poznámku",
        "Agent: 🤖 Plán → vyhledám → shrnu → uložím",
        "E.V.: Hotovo. Poznámka uložena.",
    ]),
]
...
def render_frame(title: str, lines: list[str]) -> Image.Image:
    ...
    draw.text((16, 12), title or "E.V. — Local AI OS", fill=CYAN, font=title_f)
    y = 58
    for i, line in enumerate(lines):
        color = CYAN if line.startswith("You:") else TEXT if line.startswith("E.V.:") else MUTED
